...
void calculate(uint32_t n, uint32_t m, uint32_t *edges, uint64_t *dis)
{
    ...
    constexpr uint32_t DELTA_SHIFT = 22; // 2^23 = 8,388,608
    ...
    while (true) {
        ...
        while (!R.empty()) {
            ...
            // Merge local_Rnext into R_next
            size_t total_r = 0;
            for (int t = 0; t < T; ++t) total_r += local_Rnext[t].size();
            R_next.clear();
            R_next.reserve(total_r);
            for (int t = 0; t < T; ++t) {
                auto &vec = local_Rnext[t];
                if (!vec.empty()) R_next.insert(R_next.end(), vec.begin(), vec.end());
            }
